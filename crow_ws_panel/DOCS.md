...
When a user taps an editable text field, the add-on detects the focused field
in Chromium and asks compatible panel firmware to display its on-screen
keyboard. Text, Backspace, cursor movement, Enter, and closing the keyboard are
forwarded back to the active browser field. Number and telephone inputs request
the numeric keyboard layout automatically.
